...
const QuerySidebar: React.FC<QuerySidebarProps> = ({ onSelectQuery, currentSql }) => {
    ...
    useEffect(() => {
        const savedHistory = localStorage.getItem('quackshell_history');
    ...
    const deleteHistory = (index: number) => {
        ...
        setHistory(updated);
        saveToLocalStorage('quackshell_history', updated);
    };
    ...
    return (
        <div className="flex flex-col h-full bg-surface-container-low border-r border-outline/10 w-64 animate-in slide-in-from-left duration-300">
            <div className="p-4 border-b border-outline/10 flex flex-col gap-4">
                ...
                {!isSaving ? (
                    <button
                        ...
                        className="w-full py-2.5 bg-secondary-container text-on-secondary-container rounded-2xl text-xs font-bold hover:shadow-md transition-all active:scale-95 flex items-center justify-center gap-2"
                    >
            ...
            <div className="flex-1 overflow-y-auto">
                {activeTab === 'history' ? (
                    <div className="flex flex-col p-2 gap-1">
                        {history.length === 0 ? (
                            <div className="p-8 text-center opacity-30 text-xs italic">No query history</div>
                        ) : (
                            history.map((sql, i) => (
                                <div key={i} className="group relative">
                                    <button
                                        onClick={() => setPreviewSql(sql)}
                                        className={`w-full text-left p-3 rounded-xl transition-all pr-10 border ${previewSql === sql ? 'bg-primary/10 border-primary/30 shadow-inner' : 'hover:bg-surface-variant border-transparent'}`}
                                    >
                                        <div className="text-[11px] font-mono line-clamp-2 opacity-70 group-hover:opacity-100">{sql}</div>
                                    </button>
                                    <button
                                        onClick={() => deleteHistory(i)}
                                        className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 opacity-0 group-hover:opacity-100 hover:text-error transition-all"
                                    >
                    ...
                    <div className="flex flex-col p-2 gap-1">
                        {savedQueries.length === 0 ? (
                            <div className="p-8 text-center opacity-30 text-xs italic">No saved queries</div>
                        ) : (
                            savedQueries.map((q) => (
            ...
            {previewSql && (
                <div className="p-4 bg-surface-container-high border-t border-outline/20 flex flex-col gap-3 animate-in slide-in-from-bottom duration-300">
                    <div className="flex items-center justify-between">
                        <span className="text-[10px] font-bold uppercase tracking-widest opacity-60">Query Preview</span>
                    ...
                    <button
                        ...
                        className="w-full py-2 bg-primary text-on-primary rounded-xl text-xs font-bold shadow-md hover:shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2"
                    >
                        Apply to Editor
                    </button>
                </div>
